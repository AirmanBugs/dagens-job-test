...
app.get('/products/:page', (req, res: Response) => {
  ...
  const minPrice =
    typeof req.query.minPrice === 'string' ? parseFloat(req.query.minPrice) : 0
  const filteredProducts =
    category || maxPrice < Number.MAX_SAFE_INTEGER || minPrice > 0
      ? products.filter(
          p =>
            p.category === category && p.price < maxPrice && p.price > minPrice
        )
      : products

  const page = parseInt(req.params.page)
  const firstEntry = (page - 1) * PageSize
  const lastEntry = page * PageSize
  res.send({
    status: 200,
    products: filteredProducts.slice(firstEntry, lastEntry)
  })
})
...
app.get('/products/nearestPrice/:id', (req: Request, res: Response) => {
  ...
  const productsWithDiffs = products
    .filter(product => product.id !== id)
    .map(product => ({
      ...product,
      diff: Math.abs(price - product.price)
    }))
